...
References:
    ...
    . Mark Ahn's MPU6050 Quaternion(Applied Mahony_AHRS Filter) Pitch Roll Yaw
          https://blog.naver.com/ysahn2k/221410891895

    This is an adaptation from Mark Ahn's code , with the addition of specific functions designed for our challenge solution
*/
...
#define MPU6050_AYOFFSET 45
#define MPU6050_AZOFFSET -215
...
unsigned char state; 
 float CCx, CLx, CRx, CUx, CDx;
 float CCy, CLy, CRy, CUy, CDy;
 float CCz, CLz, CRz, CUz, CDz;
...
void setup() {
  ...
  BT.begin(9600);       // Initialize BT serial port
//  
  while(b){bluetooth();}
  Serial.print(b);
  limites();
...
void loop() {
  ...
  lectura();
  if ( (pitch>minx) && (pitch<maxx) && (roll>miny) && (roll<maxy) && (yaw>minz) && (yaw<maxz))
      {}
      else beep();
...
void calibracion(int Modo) {
  if (Modo == 0) {
    Serial.print("Centered Calibration");
    Serial.println("\t");
    delay(3000);
    lecturas = 1;
    while (lecturas <= 2500) {
...
void limites(){
  ...
  float Cx[5] = {CCx, CLx, CRx, CUx, CDx};
  float Cy[5] = {CCy, CLy, CRy, CUy, CDy};
  float Cz[5] = {CCz, CLz, CRz, CUz, CDz};
